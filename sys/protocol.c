...
--*/
{
	...
	//DBGPRINT(("==> My Protocol BindAdapter\n"));
    //DBGPRINT(("==> My Protocol BindAdapter:%wZ\n", DeviceName));
	//KdPrint(("Protocol PtBindAdapter方法,DeviceName: \"%wZ\"\n", DeviceName));
    ...
    do
    {
		...
		KdPrint(("==> Protocol PtBindAdapter方法,pAdapt: 0x%p, DeviceName: \"%wZ\"\n", pAdapt, DeviceName));
...
#pragma prefast(suppress: __WARNING_POTENTIAL_BUFFER_OVERFLOW, "Ndis guarantees MediumIndex to be within bounds");
	...
	pMac = pAdapt->macAddr;
	KdPrint(("<== Protocol PtBindAdapter方法,pAdapt: 0x%p, pAdapt->macAddr: %02X-%02X-%02X-%02X-%02X-%02X, DeviceName: \"%wZ\", pAdapt->DeviceName: \"%wZ\", BindingHandle: 0x%p, MediumIndex: %d, Status: 0x%p\n",
		pAdapt, (UCHAR) pMac[0], (UCHAR) pMac[1], (UCHAR) pMac[2], (UCHAR) pMac[3], (UCHAR) pMac[4], (UCHAR) pMac[5], DeviceName,
		&(pAdapt->DeviceName), pAdapt->BindingHandle, MediumIndex, *Status));
    //DBGPRINT(("<== Protocol BindAdapter: pAdapt %p, BindingHandle %p, DeviceName %wZ, MediumIndex %d, Status %x\n", pAdapt,
	//	pAdapt->BindingHandle, &(pAdapt->DeviceName), MediumIndex, *Status));
}
...
--*/
{
	...
	KdPrint(("Protocol PtUnbindAdapter方法, 解绑网卡pAdapt: 0x%p\n", pAdapt));
    ...
    switch (NdisRequest->RequestType)
    {
      case NdisRequestQueryInformation:
        ...
        *pAdapt->BytesNeeded = NdisRequest->DATA.QUERY_INFORMATION.BytesNeeded;
		...
		{
			//    复制MAC地址到缓冲区
			if (pAdapt->bMac == FALSE)
			{
				NdisMoveMemory(&pAdapt->macAddr, NdisRequest->DATA.QUERY_INFORMATION.InformationBuffer, 6);
				pMac = pAdapt->macAddr;
				//DBGPRINT(("<== Request Complete: Adapt %p, Mac Address %X-%X-%X-%X-%X-%X\n", pAdapt,
				//	(UCHAR) pMac[0], (UCHAR) pMac[1], (UCHAR) pMac[2], (UCHAR) pMac[3], (UCHAR) pMac[4], (UCHAR) pMac[5]));
				KdPrint(("<== Protocol PtRequestComplete方法,Request完成,pAdapt: 0x%p, pAdapt->macAddr: %02X-%02X-%02X-%02X-%02X-%02X\n",
					pAdapt, (UCHAR) pMac[0], (UCHAR) pMac[1], (UCHAR) pMac[2], (UCHAR) pMac[3], (UCHAR) pMac[4], (UCHAR) pMac[5]));
				pAdapt->bMac = TRUE;
			}
        }
...
//#ifdef NDIS51
    ...
    {
		KdPrint(("协议PtSendComplete方法,这个包不属于SendPacketPool\n"));
        //
...
//#endif // NDIS51
    {
		...
		/*
		if (Pkt == 0xfafafafa) //这个包是我们转发的包，不需要维护Pki，也不需要向上层通知
		{
			KdPrint(("协议PtSendComplete方法,这个是我们的包，应该释放掉!\n"));
			Pkt = 0xfbfbfbfb;
			NdisFreePacket(Packet);
			ADAPT_DECR_PENDING_SENDS(pAdapt);
			return;
		}
		else if (Pkt == 0xfbfbfbfb)
		{
			KdPrint(("协议PtSendComplete方法,这个是我们的包，不过已经释放过了!\n"));
			return;
		}
		*/
		if (Pkt == 0xfcfcfcfc)
		{
			KdPrint(("协议PtSendComplete方法,这个包有0xfcfcfcfc特征,属于接收时修改转发的包,原来的包会丢弃\n"));
			//sendMemory发出的包，pMemory虽然是外面传过来的，也需要我们负责释放
			freePacketBufferAndMemory(Packet);
		}
		else
		
		{
			...
			if (NdisTestPacketFlags(Packet, NDIS_FLAGS_RESERVED4))

			{
				KdPrint(("协议PtSendComplete方法,这个包没有0xfcfcfcfc特征,但有NDIS_FLAGS_RESERVED4特征，属于发送时经过修改的包\n"));
				dprFreePacketBufferAndMemory(Packet);
				//NdisDprFreePacket(Packet);
			}
			else
			{
				KdPrint(("协议PtSendComplete方法,这个包没有0xfcfcfcfc特征,也没有NDIS_FLAGS_RESERVED4特征，属于发送时不经修改直接发出的包\n"));
				NdisDprFreePacket(Packet);
			}
...
{
    PADAPT				pAdapt = (PADAPT) ProtocolBindingContext;
	PADAPT				pForwardAdapt = NULL;
	FILTER_STATUS		fStatus;
	PUCHAR				pMemory;
	int					nMemLen;

	// 到达这里，说明MPTransferData返回了Pending
	//KdPrint(("协议PtTransferDataComplete方法,开始报文分析\n"));
	getMemoryCopyFromPacket(Packet, &pMemory, &nMemLen);
	fStatus = analyzeBufferEx(pAdapt, pMemory, &nMemLen, MODE_RECEIVE, &pForwardAdapt);
	//fStatus = AnalysisPacket(Packet, TRUE);
	//fStatus = FILTER_STATUS_PASS;
	if (fStatus == FILTER_STATUS_DROP)
	{
		//丢弃
		KdPrint(("协议PtTransferDataComplete方法,进入FILTER_STATUS_DROP模式\n"));
		freeMemory(pMemory);
		Status = NDIS_STATUS_FAILURE;
		return;
	}
	else if (fStatus == FILTER_STATUS_MODIFY_REDIRECT)
	{
		...
		if (pForwardAdapt)
		{
			KdPrint(("协议PtTransferDataComplete方法,进入FILTER_STATUS_MODIFY_REDIRECT模式,转发成功\n"));
			sendMemory(pForwardAdapt, pMemory, nMemLen);
		}
		else
		{
			KdPrint(("协议PtTransferDataComplete方法,进入FILTER_STATUS_MODIFY_REDIRECT模式,转发失败\n"));
			freeMemory(pMemory);
		}
		Status = NDIS_STATUS_FAILURE; //具体丢弃时间是个问题，别在转发之前就丢弃了
		return;
	}
	else if (fStatus == FILTER_STATUS_MODIFY_RECEIVE)
	{
		//修改+接收
		//不知道写的对不对，系统貌似也不怎么调用TransferData这种函数

		KdPrint(("协议PtTransferDataComplete方法,进入FILTER_STATUS_MODIFY_RECEIVE模式\n"));
		freeMemory(pMemory); //保守一点，直接打扫卫生，走人
		return;
		//freeBufferAndMemory(Packet);
		//addPacketWithMemory_ReceivePool(pAdapt, Packet, pMemory, nMemLen);
		//freePacketBufferOnly(Packet);
		//MPReturnPacket(pAdapt->BindingHandle, Packet);
		//Packet = MyPacket;
	}
	else //FILTER_STATUS_PASS
	{
		...
		//freeMemory(pMemory);
		//按PASS处理
		KdPrint(("协议PtTransferDataComplete方法,进入FILTER_STATUS_PASS模式\n"));
		freeMemory(pMemory); //保守一点，直接打扫卫生，走人
		return;
	}
...
--*/
{
    ...
    else do
    {
        ...
        if (Packet != NULL)
		{	
			//KdPrint(("协议PtReceive方法,开始报文分析 LookAheadBufferSize=%d\n", LookAheadBufferSize));
			getMemoryCopyFromPacket(Packet, &pMemory, &nMemLen);
			KdPrint(("\n==> 协议PtReceive方法,开始分析报文\n"));
			fStatus = analyzeBufferEx(pAdapt, pMemory, &nMemLen, MODE_RECEIVE, &pForwardAdapt);
			//fStatus = AnalysisPacket(Packet, TRUE);
			//fStatus = FILTER_STATUS_MODIFY_RECEIVE;
			if (fStatus == FILTER_STATUS_DROP)
			{
				//丢弃
				KdPrint(("<== 协议PtReceive方法,进入FILTER_STATUS_DROP模式\n"));
				Status = NDIS_STATUS_FAILURE;
				freeMemory(pMemory);
				return Status;
			}
			else if (fStatus == FILTER_STATUS_DROP_SEND)
			{
				//丢弃+转发
				KdPrint(("<== 协议PtReceive方法,进入FILTER_STATUS_DROP_SEND模式\n"));
				Status = NDIS_STATUS_FAILURE;
				freeMemory(pMemory);
				return Status;
			}
			else if (fStatus == FILTER_STATUS_MODIFY_REDIRECT)
			{
				...
				if (pForwardAdapt)
				{
					KdPrint(("<== 协议PtReceive方法,进入FILTER_STATUS_MODIFY_REDIRECT模式,转发成功\n"));
					sendMemory(pForwardAdapt, pMemory, nMemLen);
				}
				else
				{
					KdPrint(("<== 协议PtReceive方法,进入FILTER_STATUS_MODIFY_REDIRECT模式,转发失败\n"));
					freeMemory(pMemory);
				}
				Status = NDIS_STATUS_FAILURE; //转发时机器不会知道收到了这个包
				return Status;
			}
			else if (fStatus == FILTER_STATUS_MODIFY_RECEIVE)
			{
				...
				KdPrint(("<== 协议PtReceive方法,进入FILTER_STATUS_MODIFY_RECEIVE模式\n"));
				MyPacket = createPacketFromMemory_ReceivePool(pAdapt, pMemory, nMemLen);
				//freePacketBufferOnly(Packet);
				...
				NdisSetPacketFlags(MyPacket, NDIS_FLAGS_RESERVED4); //我们做个标记，给MPReturnPacket看的
				...
				if (pAdapt->MiniportHandle != NULL)
				{
					...
					//已经可以进行MyPacket的清理了，MPReturnPacket也不会再被调用
				}
			...
			{
				//按PASS处理
				KdPrint(("<== 协议PtReceive方法,进入FILTER_STATUS_PASS模式\n"));
				freeMemory(pMemory);
				fStatus = FILTER_STATUS_PASS;
				...
				if (Status == NDIS_STATUS_SUCCESS)
				{
					...
					//
					NdisGetPacketFlags(MyPacket) = NdisGetPacketFlags(Packet);
					NdisClearPacketFlags(MyPacket, NDIS_FLAGS_RESERVED4); //防止标记被污染
				...
				{
					KdPrint(("协议PtReceive方法,NdisDprAllocatePacket函数调用失败,按丢弃处理\n"));
					Status = NDIS_STATUS_FAILURE;
					return Status;
				}
			}
        ...
        {
			KdPrint(("协议PtReceive方法,NdisGetReceivedPacket函数调用失败,不能得到完整的包\n"));
			//不用担心，NDSI会自动调用我们的PtReceiveComplete，这里就不用管了，而且下面已经NdisMEthIndicateReceive了
            //
...
--*/
{
	...
	//KdPrint(("协议PtReceivePacket方法,开始报文分析\n"));
	getMemoryCopyFromPacket(Packet, &pMemory, &nMemLen);
	KdPrint(("\n==> 协议PtReceivePacket方法,开始分析报文\n"));
	fStatus = analyzeBufferEx(pAdapt, pMemory, &nMemLen, MODE_RECEIVE, &pForwardAdapt);
	//fStatus = AnalysisPacket(Packet, TRUE);
	//fStatus = FILTER_STATUS_MODIFY_RECEIVE;
	if (fStatus == FILTER_STATUS_DROP)
	{
		//丢弃
		KdPrint(("<== 协议PtReceivePacket方法,进入FILTER_STATUS_DROP模式\n"));
		freeMemory(pMemory);
		return 0;
	}
	else if (fStatus == FILTER_STATUS_DROP_SEND)
	{
		//丢弃+转发
		KdPrint(("<== 协议PtReceivePacket方法,进入FILTER_STATUS_DROP_SEND模式\n"));
		freeMemory(pMemory);
		return 0;
	}
	else if (fStatus == FILTER_STATUS_MODIFY_REDIRECT)
	{
		...
		if (pForwardAdapt)
		{
			KdPrint(("<== 协议PtReceivePacket方法,进入FILTER_STATUS_MODIFY_REDIRECT模式,转发成功\n"));
			sendMemory(pForwardAdapt, pMemory, nMemLen);
		}
		else
		{
			KdPrint(("<== 协议PtReceivePacket方法,进入FILTER_STATUS_MODIFY_REDIRECT模式,转发失败\n"));
			freeMemory(pMemory);
		}
		return 0; //丢弃后机器都不会知道收到了这个包
	}
	else if (fStatus == FILTER_STATUS_MODIFY_RECEIVE)
	{
		//修改+接收
		...
		KdPrint(("<== 协议PtReceivePacket方法,进入FILTER_STATUS_MODIFY_RECEIVE模式\n"));
		MyPacket = createPacketFromMemory_ReceivePool(pAdapt, pMemory, nMemLen);
		//freePacketBufferOnly(Packet);
		...
		if (pAdapt->MiniportHandle != NULL)
		{
			//不知道MyPacket是否在这里被清理，需要在MPReturnPacket里进行通过判断NDIS_STATUS_RESOURCES决定是否清理
			NdisMIndicateReceivePacket(pAdapt->MiniportHandle, &MyPacket, 1);
		}
	...
	{
		//按PASS处理
		KdPrint(("<== 协议PtReceivePacket方法,进入FILTER_STATUS_PASS模式\n"));
		freeMemory(pMemory);
		fStatus = FILTER_STATUS_PASS;
