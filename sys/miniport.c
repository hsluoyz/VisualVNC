...
--*/
{
    ...
    do
    {
        ...
        KdPrint(("==> Miniport MPInitialize方法,Miniport初始化, pAdapt: 0x%p\n", pAdapt));
    ...
    KdPrint(("<== Miniport MPInitialize方法,Miniport初始化: pAdapt: 0x%p, Status: 0x%p\n", pAdapt, Status));
...
--*/
{
...
#endif // NDIS51
    {
		...
		if (NdisTestPacketFlags(Packet, NDIS_FLAGS_RESERVED4)) ///////////////////////////此标记说明是Memory也是自己分配
		{
...
--*/
{
	...
	if (Status == NDIS_STATUS_SUCCESS)
	{
		...
		//KdPrint(("小端口MPTransferData方法,开始报文分析\n"));
		...
		{
			//丢弃
			KdPrint(("小端口MPTransferData方法,进入FILTER_STATUS_DROP模式\n"));
			freeMemory(pMemory);
			Status = NDIS_STATUS_FAILURE;
		}
		else if (fStatus == FILTER_STATUS_MODIFY_REDIRECT)
		{
			...
			if (pForwardAdapt)
			{
				KdPrint(("小端口MPTransferData方法,进入FILTER_STATUS_MODIFY_REDIRECT模式,转发成功\n"));
				sendMemory(pForwardAdapt, pMemory, nMemLen);
			}
			else
			{
				KdPrint(("小端口MPTransferData方法,进入FILTER_STATUS_MODIFY_REDIRECT模式,转发失败\n"));
				freeMemory(pMemory);
			}
			Status = NDIS_STATUS_FAILURE; //丢弃后机器都不会知道收到了这个包
		}
		else if (fStatus == FILTER_STATUS_MODIFY_RECEIVE)
		{
			//修改+接收
			//系统貌似不怎么调用TransferData这种函数

			KdPrint(("小端口MPTransferData方法,进入FILTER_STATUS_MODIFY_RECEIVE模式\n"));
			...
			NdisGetPacketFlags(MyPacket) = NdisGetPacketFlags(Packet);
			NdisSetPacketFlags(MyPacket, NDIS_FLAGS_RESERVED4); //MPReturnPacket也不会再被调用，貌似也没必要做标记了
		...
		else //FILTER_STATUS_PASS
		{
			//freeMemory(pMemory);
			//按PASS处理
			KdPrint(("小端口MPTransferData方法,进入FILTER_STATUS_PASS模式\n"));
			freeMemory(pMemory); //保守一点，直接打扫卫生，走人
			return NDIS_STATUS_FAILURE;
			/*
			//
			...
			{
				KdPrint(("小端口MPTransferData方法,NdisDprAllocatePacket函数调用失败,按丢弃处理\n"));
				Status = NDIS_STATUS_FAILURE;
				return Status;
			}
			*/
